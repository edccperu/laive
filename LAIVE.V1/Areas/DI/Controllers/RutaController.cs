...
{
   public class RutaController : SamcontrollerBase
   {
      //
      // GET: /Ruta/
      ...
      public JsonResult GetBandeja(FlexigridParamSamNet Param)
      {
         ...
         jsonR.rows = jsonR.resultArray<ERuta>(eTrans.ColumnSet(), objBO.GetByCriteria<ERuta>(eTrans));
         return Json(jsonR);
      }

      [HttpPost]
      public JsonResult UpdateModel(ERuta eRuta)
      {
         JsonMessage jmessage = new JsonMessage();

         try
         {
            IBOUpdate objBO = (IBOUpdate)WCFHelper.GetObject<IBOUpdate>(typeof(DIBOMnt.Ruta));
            objBO.UpdateData(eRuta);
