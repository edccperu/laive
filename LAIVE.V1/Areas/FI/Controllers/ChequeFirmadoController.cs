...
{
   public class ChequeFirmadoController : SamcontrollerBase
   {
      [HttpGet]
      ...
      public JsonResult GetDataGrdCheques(string filtro)
      {
         ...
         string userLogin = HttpContext.Session[ConstSessionVar.USERLOGON].ToString().Trim().ToUpper();
          
         objE.EntityFilter = JsonLaiveLib.GetWhere(filtros);
         objE.EntityFilter = string.Concat(objE.EntityFilter, (objE.EntityFilter == "" ? "" : " AND "), "(( a.codigoEstado in ('E05','E08') AND upper(ltrim(rtrim(c.login))) = '", userLogin, "') OR ( a.codigoEstado in ('E06','E08') AND upper(ltrim(rtrim(d.login))) = '", userLogin, "'))");
      ...
      public JsonResult CambiarEstado(string[] chequeIds, string dsObservacion, string newEstado)
      {
         ...
         try
         {
            ...
            foreach (string idCheque in chequeIds)
            {
               ...
               eChequeLog.Observacion = string.Concat("CHEQUE REVERTIDO POR EL FIRMANTE : ", dsObservacion);
               ...
               if (newEstado == ConstEstadoCheque.EXCLUIDO)
                  objBO.RevertirChequeFirmado(eChequeLog);
