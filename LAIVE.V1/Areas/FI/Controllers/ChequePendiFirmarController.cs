...
{
   public class ChequePendiFirmarController : SamcontrollerBase
   {
      //
      // GET: /ChequePendiFirmar/
      ...
      public JsonResult GetDataGrdCheques(string filtro)
      {
         ...
         ICollection<ECheque> listCheque = objBO.GetChequeXFirmar(objE);
      ...
      public JsonResult CambiarEstado(string[] chequeIds, string dsObservacion, string newEstado)
      {
         ...
         try
         {
            ...
            foreach (string idCheque in chequeIds)
            {
               ...
               eChequeLog.Observacion = string.Concat("CHEQUE RECHAZADO POR EL FIRMANTE : ",dsObservacion);
               ...
               if(newEstado == ConstEstadoCheque.EXCLUIDO)
                 objBO.RechazarCheque(eChequeLog);
               else
                 objBO.FirmarCheque(eChequeLog);

            }
            FIBOQry.ICheque objBOE = (FIBOQry.ICheque)WCFHelper.GetObject<FIBOQry.ICheque>(typeof(FIBOQry.Cheque));
            ECheque objE = new ECheque();
            List<FilterOperator> filtros = new List<FilterOperator>();
            filtros.Add(new FilterOperator("a.idCheque",EnumConectorFilter.IN,chequeIds));

            objE.Login = HttpContext.Session[ConstSessionVar.USERLOGON].ToString();
            objE.EntityFilter = JsonLaiveLib.GetWhere(filtros);
            //ICollection<ECheque> listCheque = objBOE.GetChequeEnvioEmail(objE);
